...
你可以在Ultimate-Starter-Kit-for-Pico-W/1.Python/iot中找到这个文件
...
--------------------------------------------------------------
``do_connect.py``是一个简单的脚本，它使用WLAN对象连接到WiFi网络,我们将它设置为一个单独的文件,每次需要使用的时候直接调用它就行了

你可以在Ultimate-Starter-Kit-for-Pico-W/1.Python/iot中找到这个文件,我们需要将它上传到pico w中
